...
function init_ip_rechner(container) {
    ...
    I18N.register('ip', {
        de: {
            ...
            'localIpToggle':'Lokale IP finden?',
            'localIpHint':  'Browser k\u00f6nnen die lokale IP nicht auslesen. So findest du sie:',
        ...
        en: {
            ...
            'localIpToggle':'Find local IP?',
            'localIpHint':  'Browsers cannot read the local IP. How to find it:',
    ...
    container.innerHTML = `
        <section class="card input-card">
            ...
            <a class="ip-local-toggle" id="ip-local-toggle" href="#">${t('ip.localIpToggle')}</a>
            <div class="ip-local-hint" id="ip-local-hint" style="display:none;">
    ...
    const localToggle = document.getElementById('ip-local-toggle');
    ...
    // Local IP hint toggle
    localToggle.addEventListener('click', (e) => {
        e.preventDefault();
        localHint.style.display = localHint.style.display === 'none' ? 'block' : 'none';
    });
